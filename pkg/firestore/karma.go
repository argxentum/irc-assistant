...
func (fs *Firestore) KarmaHistory(ctx context.Context, channel, userID string) ([]*models.KarmaHistory, error) {
	path := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s", pathAssistants, fs.cfg.Connection.Nick, pathChannels, channel, pathUsers, userID, pathKarmaHistory)
...
func (fs *Firestore) AddKarmaHistory(ctx context.Context, channel, from, to, op, reason string) (int, error) {
	...
	path := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s/%s", pathAssistants, fs.cfg.Connection.Nick, pathChannels, channel, pathUsers, u.ID, pathKarmaHistory, karmaHistory.ID)
