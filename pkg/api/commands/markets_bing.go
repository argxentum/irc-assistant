...
const BingMarketsCommandName = "markets_bing"

type BingMarketsCommand struct {
	*commandStub
	retriever retriever.DocumentRetriever
}

func NewBingMarketsCommand(ctx context.Context, cfg *config.Config, irc irc.IRC) Command {
	return &BingMarketsCommand{
		commandStub: defaultCommandStub(ctx, cfg, irc),
		retriever:   retriever.NewDocumentRetriever(retriever.NewBodyRetriever()),
	}
}

func (c *BingMarketsCommand) Name() string {
	return BingMarketsCommandName
}

func (c *BingMarketsCommand) Description() string {
	return "Displays current stock market data for the given region. Defaults to US."
}

func (c *BingMarketsCommand) Triggers() []string {
	return []string{"markets", "market"}
}

func (c *BingMarketsCommand) Usages() []string {
	return []string{"%s", "%s <region>"}
}

func (c *BingMarketsCommand) AllowedInPrivateMessages() bool {
	return true
}

func (c *BingMarketsCommand) CanExecute(e *irc.Event) bool {
	return c.isCommandEventValid(c, e, 0)
}

func (c *BingMarketsCommand) Execute(e *irc.Event) {
...
func (c *BingMarketsCommand) retrieveSummary(e *irc.Event, region string) string {
	logger := log.Logger()
