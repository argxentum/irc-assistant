...
func (c *SummaryCommand) parseShortcut(e *irc.Event, url string) (*summary, *models.Source, error) {
	...
	if len(source) == 0 {
		return nil, nil, fmt.Errorf("can't find shortcut with source %s", id)
	}

	if !urlRegex.MatchString(source) {
		return nil, nil, fmt.Errorf("invalid URL: %s", source)
	}

	if len(e.Arguments) < 2 {
		return nil, nil, fmt.Errorf("missing URL argument")
	}
